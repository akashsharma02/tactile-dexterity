...
from tactile_dexterity.tactile_data.tactile_image import TactileImage
from tactile_dexterity.utils import VISION_IMAGE_MEANS, VISION_IMAGE_STDS, load_data


class TactileVisionActionDataset(data.Dataset):
    def __init__(
        self, data_path, tactile_information_type, tactile_img_size, vision_view_num
    ):
        super().__init__()
        ...
        self.tactile_img = TactileImage(
            tactile_image_size=tactile_img_size, shuffle_type=None
        )

    def _preprocess_tactile_indices(self):
        self.tactile_mapper = np.zeros(
            len(self.data["tactile"]["indices"]) * 15
        ).astype(int)
        for data_id in range(len(self.data["tactile"]["indices"])):
            for sensor_id in range(15):
                self.tactile_mapper[
                    data_id * 15 + sensor_id
                ] = data_id  # Assign each finger to an index basically
    ...
    def _get_image(self, index):
        ...
        image_path = os.path.join(
            image_root,
            "cam_{}_rgb_images/frame_{}.png".format(
                self.vision_view_num, str(image_id).zfill(5)
            ),
        )
    ...
    def _get_tactile_image(self, tactile_values):
        return self.tactile_img.get(
            type=self.tactile_information_type, tactile_values=tactile_values
        )
    ...
    def _get_action(self, index):
        ...
        allegro_action = self.data["allegro_actions"]["values"][demo_id][
            allegro_action_id
        ]
        ...
        return torch.FloatTensor(
            total_action
        )  # These values are already quite small so we'll not normalize them
