...
from holodex.utils.network import ZMQCameraSubscriber


def plot_tactile_sensor(
    ax, sensor_values, use_img=False, img=None, title="Tip Position"
):
...
def dump_robot_state(allegro_tip_pos, kinova_cart_pos):
    ...
    plot_xyz_position(
        ax=kinova_ax,
        position=kinova_cart_pos,
        title=f"Arm Wrist Position",
...
def dump_knn_state(
    ...
    img_name,
    image_repr=False,
    add_repr_effects=False,
    include_temporal_states=False,
):  # image_repr - if image is part of repr we only show tactile and image
    ...
    if not image_repr:
        ...
        if include_temporal_states:
            ...
            prev_next_knn_state_img = concat_imgs(
                prev_knn_state, next_knn_state, "horizontal"
            )
            all_state_img = concat_imgs(
                curr_knn_state_img, prev_next_knn_state_img, "vertical"
            )
        else:
...
def dump_repr_effects(
    nn_separate_dists, viz_id_of_nns, demo_nums, representation_types
):  # nn_separate_dists has all the dists from the closest applicable one to the rest
