...
def dump_video_to_images(
    root: str, video_type: str = "rgb", view_num: int = 0, dump_all=True
) -> None:
    ...
    while success:
        pbar.update(1)
        if (
            not dump_all and frame_id == desired_indices[desired_img_id][1]
        ) or dump_all:
            cv2.imwrite(
                "{}.png".format(
                    os.path.join(images_path, "frame_{}".format(str(frame_id).zfill(5)))
                ),
                image,
            )
            curr_id = desired_img_id
            while (
                desired_img_id < len(desired_indices) - 1
                and desired_indices[curr_id][1] == desired_indices[desired_img_id][1]
            ):
                desired_img_id += 1
        success, image = vidcap.read()
        frame_id += 1
...
def dump_fingertips(root):
    ...
    fingertip_states = (
        dict()
    )  # Will have timestamps as the allegro_joint_states and the tip positions of each finger
    ...
    with h5py.File(fingertip_state_file, "w") as file:
        for key in fingertip_states.keys():
            if key == "timestamps":
                fingertip_states[key] = np.array(
                    fingertip_states[key], dtype=np.float64
                )
            else:
                fingertip_states[key] = np.array(
                    fingertip_states[key], dtype=np.float32
                )

            file.create_dataset(
                key, data=fingertip_states[key], compression="gzip", compression_opts=6
            )
...
def dump_data_indices(
    ...
    allegro_commanded_joint_path = os.path.join(
        root, "allegro_commanded_joint_states.h5"
    )
    ...
    allegro_action_id = get_closest_id(
        allegro_action_id, tactile_timestamp, allegro_action_timestamps
    )
    ...
    while True:
        if is_byol_tactile:
            ...
            if find_steps_separately:
                ...
                if pos_allegro_id >= len(allegro_positions) or pos_kinova_id >= len(
                    kinova_positions
                ):
                    break
                metric_timestamp = min(
                    kinova_timestamps[pos_kinova_id], allegro_timestamps[pos_allegro_id]
                )
        ...
        allegro_action_id = get_closest_id(
            allegro_action_id, metric_timestamp, allegro_action_timestamps
        )
        ...
        if (
            image_id >= len(image_timestamps) - 1
            or tactile_id >= len(tactile_timestamps) - 1
            or allegro_id >= len(allegro_timestamps) - 1
            or allegro_action_id >= len(allegro_action_timestamps) - 1
            or kinova_id >= len(kinova_timestamps) - 1
        ):
            break
    ...
    assert (
        len(tactile_indices) == len(allegro_indices)
        and len(tactile_indices) == len(image_indices)
        and len(tactile_indices) == len(allegro_action_indices)
        and len(tactile_indices) == len(kinova_indices)
    )

    # Save the indices for that root
    last_stopping_index = (
        1  # Not including the last few frames since they are noisy a bit
    )
...
def find_next_allegro_id(kdl_solver, positions, pos_id, threshold_step_size=0.01):
    ...
    for i in range(pos_id, len(positions)):
        ...
        step_size = np.linalg.norm(
            old_allegro_fingertip_pos - curr_allegro_fingertip_pos
        )
...
def find_next_robot_state_timestamp(
    kdl_solver,
    allegro_positions,
    allegro_timestamps,
    allegro_id,
    ...
    for i in range(allegro_id, len(allegro_positions)):
        curr_allegro_fingertip_pos = kdl_solver.get_fingertip_coords(
            allegro_positions[i]
        )
        ...
        step_size = np.linalg.norm(
            old_allegro_fingertip_pos - curr_allegro_fingertip_pos
        ) + np.linalg.norm(old_kinova_pos - curr_kinova_pos)
        if step_size > threshold_step_size:
